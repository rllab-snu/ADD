...
class EncoderLinearAttentionModel(nn.Module):
    def __init__(
        ...
        self.attention_layer = SimpleSelfAttention(hidden_dim)
        self.emb_layer_2 = nn.Linear(hidden_dim, hidden_dim)
        ...
        self.emb_layer_4 = nn.Linear(hidden_dim, hidden_dim)
    ...
    def forward(self, input, timesteps):
        ...
        x = self.attention_layer(x + emb.unsqueeze(dim=1))
        x = x.sum(dim=1)
        x = self.activation(x)
        emb = self.emb_layer_2(emb)
        emb = self.activation(emb)
...
class EncoderUNetModel(nn.Module):
    ...
    def forward(self, x, timesteps=None):
        ...
        if self.dims == 1:
            ...
            return self.out(h)
        elif self.pool.startswith("spatial"):
            results.append(h.type(x.dtype).mean(dim=(2, 3)))
