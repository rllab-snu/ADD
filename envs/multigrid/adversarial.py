...
class AdversarialEnv(multigrid.MultiGridEnv):
  ...
  def reset_to_level(self, level, use_generator=False, agent_dir=0):
    self.reset()
    ...
    if isinstance(level, str):
      ...
      if self.resample_n_clutter:
        ...
        if use_generator:
            ...
            self.agent_dir=[agent_dir] * self.n_agents
  ...
  def reset_to_generated_img(self, generated_img):
      level = ""
      guided_env_wo_padding = generated_img[:-1, :-1]
      guided_env_inner = generated_img[1:-2, 1:-2]
      wall_pos_idx_x, wall_pos_idx_y = np.where(guided_env_inner[:, :, 0] > 253)
      for wall_idx in range(min(len(wall_pos_idx_x), self.n_clutter - 1)):
          level += str(wall_pos_idx_x[wall_idx] * 13 + wall_pos_idx_y[wall_idx]) + " "
      
      goal_pos_idx = np.argmax(guided_env_inner[:, :, 2])
      level += str(goal_pos_idx) + " "
      
      start_pos_idx = np.argmax(guided_env_inner[:, :, 1])
      level += str(start_pos_idx)
      
      start_pos_idx_x = start_pos_idx // 13 + 1
      start_pos_idx_y = start_pos_idx % 13 + 1
      
      agent_direction_x_sub = [0, 1, 0, -1]
      agent_direction_y_sub = [1, 0, -1, 0]
      
      direction = np.argmax(guided_env_wo_padding[agent_direction_x_sub + start_pos_idx_x, 
                                                      agent_direction_y_sub + start_pos_idx_y, 1])
      
      obs = self.reset_to_level(level=level, use_generator=True, agent_dir=direction) # 0: right, 1: down, 2: left, 3: up 
  ...
  def step_adversary(self, loc):
    ...
    # End of episode
    # if use_generator:
    #     done_threshold = self.adversary_max_steps
...
class GoalLastVariableBlocksAdversarialADDEnv(AdversarialEnv):
  def __init__(self, fixed_environment=False, seed=None):
    super().__init__(
      choose_goal_last=True, n_clutter=70, resample_n_clutter=True,
      fixed_environment=fixed_environment, seed=seed, max_steps=250)
...
register.register(
    env_id='MultiGrid-GoalLastVariableBlocksAdversarialEnv-v7',
    entry_point=module_path + ':GoalLastVariableBlocksAdversarialADDEnv',
    max_episode_steps=250,
)
