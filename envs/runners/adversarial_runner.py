...
class AdversarialRunner(object):
    ...
    def __init__(
        ...
        self.use_generator = args.ued_algo == "generative_model"
        if self.use_generator:
            self.generator = EnvGenerator(args)
            # self.generator.create_initial_noise_schedule(args)
            self.tutor_list = None
            if args.use_guidance:
                self.tutor_list = []
                for i in range(args.num_tutor):
                    tutor = Tutor(args)
    ...
    def agent_rollout(
        ...
        if is_env:
            ...
            if level_replay:  # Get replay levels
                ...
                generated_envs = self.generator.generate_env(
                    args.num_processes, tutor_list=self.tutor_list
                )
                obs = self.ued_venv.reset_to_generated_imgs(generated_envs)
                self.total_seeds_collected += args.num_processes
                return
        ...
        rollout_failures = [[] for _ in range(args.num_processes)]
        ...
        for step in range(num_steps):
            ...
            for i, info in enumerate(infos):
                if "episode" in info.keys():
                    ...
                    if reward[i] <= self.failure_reward:
                        rollout_failures[i].append(True)
                    else:
                        rollout_failures[i].append(False)
        ...
        rollout_info.update({"failures": rollout_failures})
    ...
    def _compute_env_return(self, agent_info, adversary_agent_info, adversary_env):
        args = self.args
        ...
        elif args.ued_algo == "generative_model" and args.use_guidance:
            for tutor in self.tutor_list:
                if tutor.categorical:
                    tutor.add_result(agent_info["returns"], agent_info["failures"])
                else:
                    tutor.add_result(agent_info["mean_return"])
                tutor.update()
                # tutor.update(agent_info['mean_return'])
    ...
    def run(self):
            ...
            if self.base_levels == "batch":
                ...
                if args.num_processes >= 4:
                    ...
                    easy = np.argmax(
                        (
                            agent_info["mean_return"].detach().cpu().numpy()
                            - agent_info["batched_value_loss"].detach().cpu().numpy()
                        ).flatten()
                    )
        ...
        env_return = self._compute_env_return(
